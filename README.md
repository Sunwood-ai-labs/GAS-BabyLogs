...
- 📥 **イベント抽出**: 指定した共有カレンダーから過去〜未来のイベントを取得し、カテゴリ判定（うんち/しっこ/両方）を行います。
- 🧹 **データ整形**: 取得したイベントを `baby_logs` シートへ書き込み、ヘッダー固定・フィルタ・自動列調整を実施します。
- 📊 **集計 & グラフ**: `baby_summary` シートに日別・月別テーブルとグラフ 3 種（直近 30 日の積み上げ棒、月別棒、カテゴリ内訳円グラフ）を生成します。
